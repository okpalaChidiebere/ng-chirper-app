...
let tweets: Record<string, Tweet> = {
  '8xf0y6ziyjabvozdd253nd': {
...
export function _getTweets(): Promise<Record<string, Tweet>> {
  return new Promise((res, rej) => {
    setTimeout(() => res({ ...tweets }), 1000);
  });
}

export function _saveLikeToggle({ id, hasLiked, authedUser }): Promise<void> {
  return new Promise((res, rej) => {
    setTimeout(() => {
      tweets = {
        ...tweets,
...
export function _saveTweet({ text, author, replyingTo }): Promise<Tweet> {
  return new Promise((res, rej) => {
