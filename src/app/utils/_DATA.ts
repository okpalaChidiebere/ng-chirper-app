...
export type SaveLikeToggleRequest = {
  id: string;
  author: string;
  has_liked: boolean;
...
export type SaveTweetRequest = Pick<Tweet, 'text' | 'author'> & {
  replying_to: string;
};
...
export function _getUsers(): Promise<Array<User>> {
  return new Promise((res, rej) => {
    setTimeout(() => res(users), 1000);
  });
}

export function _getTweets(): Promise<Array<Tweet>> {
  return new Promise((res, rej) => {
...
export function _saveLikeToggle({
  ...
  authed_user_id,
  author,
}: SaveLikeToggleRequest): Promise<void> {
  return new Promise((res, rej) => {
    setTimeout(() => {
      tweets = tweets.map((tweet) =>
        tweet.id !== id && tweet.author !== author
          ? tweet
          : Object.assign({}, tweet, {
              likes:
                has_liked === true
                  ? tweet.likes?.filter((uid) => uid !== authed_user_id)
                  : tweet.likes?.concat([authed_user_id]) ?? [authed_user_id],
            })
      );
...
function formatTweet({ author, text, replyingTo = null }): Tweet {
  return {
    author,
    id: generateUID(),
    likes: [],
...
export function _saveTweet({
  ...
  author,
  replying_to,
}: SaveTweetRequest): Promise<Tweet> {
  return new Promise((res, rej) => {
    ...
    setTimeout(() => {
      tweets = tweets
        .map((tweet) =>
          formattedTweet.replyingTo && tweet.id === formattedTweet.replyingTo
            ? Object.assign({}, tweet, {
                replies: tweet.replies?.concat([formattedTweet.id]) ?? [
                  formattedTweet.id,
                ],
              })
            : tweet
        )
        .concat([formattedTweet]);

      users = users.map((user) =>
        user.id == author
          ? Object.assign({}, user, {
              tweets: user.tweets?.concat([formattedTweet.id]) ?? [
                formattedTweet.id,
              ],
            })
          : user
